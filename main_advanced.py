...
app = FastAPI(
    title="MoviePy Video Generator ADVANCED",
    description="Sistema completo para videos motivacionales con imágenes, voz y efectos",
    version="3.0.0"  # ADVANCED VERSION
)
...
async def health_check():
    ...
    return {
        "status": "healthy", 
        "service": "moviepy-video-generator-advanced",
        "version": "3.0.0",
        "renderer": "Advanced FFmpeg with Effects",
...
async def root():
    ...
    return {
        "message": "MoviePy Video Generator ADVANCED",
        "version": "3.0.0",
        "renderer": "Advanced FFmpeg with zoom, text, audio mixing",
...
async def create_advanced_video(request: AdvancedVideoRequest, video_id: str) -> str:
    """ADVANCED video creation with images, zoom effects, text overlays and audio"""
    temp_dir = None
    ...
    try:
        ...
        # STEP 3: Create complex ffmpeg filter chain
        ...
        for image_path in image_files:
            ffmpeg_inputs.extend(["-loop", "1", "-t", str(request.scenes[len(ffmpeg_inputs)//4].duration), "-i", image_path])
...
async def generate_advanced_video(request: AdvancedVideoRequest, background_tasks: BackgroundTasks):
    ...
    return VideoResponse(
        ...
        duration=total_duration,
        status="queued",
        message=f"ADVANCED video generation started - {len(request.scenes)} scenes with effects v3.0"
    )
